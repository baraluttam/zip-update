...
mysql --host=$DBHOST --user=$DBUSERNAME --password=$DBPASSWORD TEST_Validation_tmp < *MYSQL-STANDARD.sql
echo "finished importing zip-codes-database-MYSQL-STANDARD.sql"
echo "importing zip-codes-database-STANDARD.sql"
mysql --host=$DBHOST --user=$DBUSERNAME --password=$DBPASSWORD TEST_Validation_tmp < *database-STANDARD.sql
